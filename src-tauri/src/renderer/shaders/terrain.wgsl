...
fn sample_height(uv: vec2<f32>) -> f32 {
    return textureSample(t_heightmap, s_linear, uv).r;
}
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // Colour from hypsometric ramp (normalized height → ramp U coordinate)
    let ramp_u = clamp(h, 0.0, 1.0);
    var color = textureSample(t_color_ramp, s_linear, vec2<f32>(ramp_u, 0.5)).rgb;
    ...
    // River overlay from flow accumulation
    var final_color = shaded;
    if u.has_flow > 0.5 && h >= u.sea_level {
        let flow = textureSample(t_flow, s_linear, uv).r;
        if flow > 0.15 {
